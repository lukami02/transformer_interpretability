...
Metode interpretabilnosti veštačke inteligencije (\textit{Explainable AI} - XAI) mogu se fundamentalno podeliti na osnovu stepena pristupa unutrašnjoj arhitekturi i parametrima modela. Ovo poglavlje posvećeno je metodama specifičnim za model (\textit{white-box methods}) koje zahtevaju potpun uvid u unutrašnje stanje mreže, uključujući njene težine, aktivacije i gradijente. Nasuprot njima, metode nezavisne od modela, koje sistem posmatraju isključivo spolja, biće detaljno analizirane u narednom poglavlju.
...
Kod CNN arhitektura primena je direktna: konvolucioni slojevi čuvaju lokalnu prostornu 
...
\end{equation}
Rezultujući vektor od $N$ elemenata direktno odgovara pečevima na koje je ulazna slika 
...
Iako sabiranje po definiciji čuva relevantnost (dokaz u Dodatku \ref{appendix:addition}), ono unosi rizik od numeričke nestabilnosti. Ukoliko se komponente ulaza međusobno poništavaju ($u_j + v_j \approx 0$), imenilac teži nuli, što dovodi do deljenje vrednošću bliskom nuli i nekontrolisane eksplozije relevantnosti.
...
\paragraph{Softmax i Layer Normalization} Za slojeve poput softmax i layer normalization, relevantnost se propagira nepromenjeno. Ovi slojevi se u LRP propagaciji tretiraju kao identičko preslikavanje (\textit{identity mapping}) \cite{chefer2021transformer}.
...
Međutim, kako se različite komponente pažnje specijalizuju za različite vrste 
zavisnosti \cite{voita2019analyzing}, jednostavnim usrednjavanjem gube se 
ove specijalizovane informacije i sve komponente se tretiraju kao podjednako informativne.

Pored toga, Attention Rollout je osetljiv na problem zasićenja 
...
Na ovaj način se čuva konzervacija relevantnosti kroz slojeve, u skladu sa LRP principom. Bez normalizacije, slojevi sa većim gradijentnim doprinosom mogli bi nesrazmerno da dominiraju pri matričnom množenju kroz blokove, što bi narušilo ravnotežu propagacije.
